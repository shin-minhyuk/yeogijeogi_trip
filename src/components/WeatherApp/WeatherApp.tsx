...
import { API_KEY, BASE_URL, getBaseDate, getBaseTime, getPageNo } from '../../utils/weatherApi';
...
export default function WeatherApp({ mapX, mapY }: ILocationDataProps) {
  ...
  const fetchWeatherData = async () => {
    if (coordinates.x && coordinates.y) {
      ...
      const response = await fetch(
        `${BASE_URL}/getVilageFcst?serviceKey=${API_KEY}&pageNo=${pageNo}&numOfRows=12&dataType=JSON&base_date=${baseDate}&base_time=${baseTime}&nx=${coordinates.x}&ny=${coordinates.y}`
      );
  ...
  // 날씨 정보 필터링 함수
  const filterWeatherInfo = (): { weather: string; temperature: string | undefined } => {
    const filteredSkyState = weatherData?.find((data) => data.category === 'SKY');
    const skyState = filteredSkyState?.fcstValue;

    const filteredRainSnowState = weatherData?.find((data) => data.category === 'PTY');
    const rainSnowState = filteredRainSnowState?.fcstValue;

    const filteredTempState = weatherData?.find((data) => data.category === 'TMP');
  ...
  const weatherDataObj = weatherData.length > 0 ? { id: 1, day: '오늘', ...filterWeatherInfo() } : null;
  ...
  return (
    <>
      {loading ? (
        ...
        <div className="fixed bottom-2 right-2">날씨 정보를 가져오지 못했습니다.</div>
      )}
    </>
  );
}
